...
const DECISION_OPTIONS = [
  ...
  { value: "a_ameliorer", label: "⚠️ À améliorer" },
      ...
      <div className="p-6 border-t border-blue-gray-100">
        <Link to="/auth/sign-in">
          <Button
            ...
            variant="text"
            size="sm"
            className="flex items-center justify-center gap-2 bg-red-50 hover:bg-red-100 active:bg-red-200"
          >
            <svg
              className="w-4 h-4"
              fill="none"
    ...
    <Dialog open={open} handler={onClose} size="lg" className="bg-white rounded-2xl">
      <DialogHeader className="border-b border-blue-gray-50 p-6">
        <div className="modal-header" style={{ width: "100%", margin: 0 }}>
          <div>
            <p className="modal-title font-black text-slate-900">
              {application?.student?.name}
            </p>
            <p className="modal-subtitle text-slate-500">
              Détails de la candidature et validation
        ...
        {application && (
          <>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm">
              <p className="font-bold text-slate-900 mb-3 text-sm flex items-center gap-2">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {getCvUrl(application.cv_path || application.cv) && (
                  <div>
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-wide mb-1">
                      Curriculum Vitae
                    </p>
                    <a
                      href={getCvUrl(application.cv_path || application.cv)}
                      target="_blank"
            ...
            {isAcceptedOrCompleted ? (
              <div className="bg-white rounded-xl border border-blue-200 p-5 shadow-sm">
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        onEvalFormChange("score", e.target.value)
                      }
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        onEvalFormChange("final_decision", e.target.value)
                      }
                      className="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition"
                    >
                ...
                <div className="mt-4 flex justify-end">
                  <Button
                    onClick={() => onSaveEvaluation(application.id)}
              ...
              <div className="bg-slate-50 border border-slate-200 border-dashed rounded-xl p-5 text-center">
                ...
                <p className="text-xs text-slate-400 mt-1">
                  La validation s'active uniquement lorsque le statut de la
...
export default function ManagerApplications() {
  ...
  /* ── Fetch Evaluation ── */
  const fetchEvaluation = useCallback(async (appId) => {
    try {
      ...
      console.error("Erreur fetch evaluation:", err);
      setEvalForm({
        score: "",
        final_decision: "pending",
        notes: "",
    ...
    async (applicationId) => {
      try {
        ...
        const res = await api.put(
          `/rh/applications/${applicationId}/evaluation`,
          {
        ...
        console.error("Erreur save evaluation:", err);
      } finally {
  ...
  const handleEvalFormChange = useCallback((field, value) => {
  ...
  useEffect(() => {
    filterApplications();
  }, [applications, search, filterApplications]);
