...
function RichCommentInput({ value, onChange, onSend, busy, placeholder = "Votre commentaire…" }) {
  ...
  return (
    <div className="border border-slate-200 rounded-xl overflow-hidden bg-white focus-within:ring-2 focus-within:ring-indigo-200 focus-within:border-indigo-400 transition shadow-sm">
      {showFormatBar && (
      ...
      <div className="flex items-center gap-1 px-3 py-2 border-t border-slate-100 bg-slate-50/60">
        <button
          ...
          className={`w-8 h-8 rounded-lg flex items-center justify-center transition ${showFormatBar ? "bg-indigo-100 text-indigo-600" : "text-slate-400 hover:bg-slate-200 hover:text-slate-600"}`}
        >
        ...
        <button
          ...
          className="ml-auto px-4 py-2 bg-indigo-600 text-white text-sm font-bold rounded-lg hover:bg-indigo-700 disabled:opacity-40 disabled:cursor-not-allowed transition active:scale-95 flex items-center gap-2"
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function CommentBubble({ comment, isEncadrant, isCurrentUser, onDelete, onEdit }) {
  ...
  const name = comment.encadrant_id ? "Moi" : (comment.user?.name || comment.student?.name || "Étudiant");
  ...
  const bubbleClass = comment.encadrant_id
    ? "bg-gradient-to-r from-indigo-50 to-purple-50 border-indigo-100"
    : "bg-slate-50 border-slate-200";
  const avatarClass = comment.encadrant_id
    ? "bg-indigo-600"
    : "bg-slate-600";
  const nameClass = comment.encadrant_id ? "text-indigo-900" : "text-slate-900";
        ...
        {isCurrentUser && !isTemp && (
          <div className="flex items-center gap-1 ml-2">
            <button
              ...
              title="Modifier"
              className="w-7 h-7 rounded flex items-center justify-center text-slate-400 hover:bg-white hover:text-indigo-500 transition"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
      ...
      {editing ? (
        <div className="ml-10.5 space-y-2 mt-1">
          <textarea
            ...
            rows={3}
            className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-indigo-200 resize-none"
          />
          <div className="flex gap-2">
            <button type="button" onClick={handleSaveEdit}
              className="px-3 py-1.5 text-xs font-bold bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition">
              Enregistrer
            </button>
            ...
            <div className="ml-10 mt-3 flex flex-wrap gap-2.5">
              {attachments.map((att, i) => (
                att.type?.startsWith("image/") && att.url ? (
                  ...
                  <a key={i} href={getStorageUrl(att.url)} target="_blank" rel="noreferrer"
                    className="flex items-center gap-1.5 bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs text-indigo-600 font-medium hover:bg-indigo-50 transition shadow-sm">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                        d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
                    </svg>
...
export default function EncadrantTaskDetail() {
  ...
  const loadData = useCallback(async (silent = false) => {
    ...
    try {
      const [resApp, resTasks, resUser] = await Promise.all([
        api.get(`/encadrant/supervision/applications/${applicationId}`),
        api.get(`/encadrant/applications/${applicationId}/tasks?per_page=50`),
        api.get("/user/profile").catch(() => ({ data: JSON.parse(localStorage.getItem("user") || "{}") }))
      ]);

      const app = resApp.data;
      ...
      const foundTask = tasksArray.find(t => String(t.id) === String(taskId));

      if (foundTask) {
        const resComments = await api.get(`/encadrant/tasks/${taskId}/comments`);
        ...
        navigate(`/enterprise/encadrant/student/${applicationId}`);
      }
    } catch (error) {
  ...
  const sendComment = async (attachments = []) => {
    ...
    const tempComment = {
      ...
      encadrant_id: userData?.id, // Marker for "Moi"
    ...
    try {
      ...
      attachments.forEach(att => formData.append("attachment[]", att.file));

      await api.post(
        `/encadrant/tasks/${taskId}/comments`,
        formData,
  ...
  const deleteComment = async (commentId) => {
    ...
    try {
      await api.delete(`/encadrant/comments/${commentId}`);
    } catch (e) {
      console.error(e);
      loadData(true);
    }
  ...
  const editComment = async (commentId, body) => {
    ...
    try {
      await api.put(`/encadrant/comments/${commentId}`, { body });
    } catch (e) {
      console.error(e);
      loadData(true);
    }
  ...
  const deleteTask = async () => {
    ...
    try {
      await api.delete(`/encadrant/tasks/${taskId}`);
  ...
  const patchStatus = async (status) => {
    ...
    try {
      await api.put(`/encadrant/tasks/${taskId}`, { status });
      Swal.fire({
    ...
    <BaseLayout
      ...
      sidebarHeader={
        <EnterpriseSidebarHeader
          ...
          roleConfig={{ label: "Encadrant", color: "purple", icon: "🎓" }}
          name={userData?.name}
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-4 rounded-xl shadow-sm border border-slate-200">
          <div>
            <Breadcrumbs className="bg-transparent p-0 mb-1">
              <Link to="/enterprise/encadrant" className="opacity-60 hover:text-indigo-600 transition">Tableau de bord</Link>
              <Link to={`/enterprise/encadrant/student/${applicationId}`} className="opacity-60 hover:text-indigo-600 transition">
                Suivi : {student?.name || "Étudiant"}
              </Link>
            ...
            <Typography variant="small" className="text-slate-500 font-medium">
              Suivi de stage • Assigné à {student?.name || "Étudiant"}
            </Typography>
          </div>
          <Link
            to={`/enterprise/encadrant/student/${applicationId}`}
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
              ...
              <div className="p-6">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className={`w-9 h-9 rounded-xl border flex items-center justify-center transition
                        ${showEdit ? "border-indigo-400 bg-indigo-50 text-indigo-600 shadow-inner" : "border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-indigo-600"}`}
                    >
                ...
                {/* Mode Edition */}
                {showEdit ? (
                  <div className="border border-indigo-200 rounded-xl bg-indigo-50/40 p-5 mt-4 space-y-4">
                    <p className="text-sm font-bold text-indigo-800 uppercase tracking-wider mb-2 flex items-center gap-2">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 bg-white"
                        />
                      </div>
                      <div>
                        ...
                        <textarea
                          ...
                          rows={4}
                          className="w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 bg-white resize-y"
                        />
                      </div>
                      <div className="flex gap-4">
                        ...
                        <div className="flex-1">
                          ...
                          <button type="button" onClick={() => imgRef.current?.click()}
                            className="w-full flex items-center justify-center gap-2 border border-slate-300 bg-white rounded-lg px-3.5 py-2.5 text-sm text-slate-700 hover:bg-slate-50 hover:border-indigo-300 transition">
                      ...
                      <div className="flex gap-3 pt-2">
                        <button
                          ...
                          className="flex-1 py-2.5 rounded-lg bg-indigo-600 text-white text-sm font-bold hover:bg-indigo-700 disabled:opacity-40 transition flex items-center justify-center gap-2"
                        >
                      ...
                      <div className="mt-5">
                        ...
                        {task.attachment.match(/\.(jpeg|jpg|gif|png|webp)$/i) ? (
                          ...
                          <a href={getStorageUrl(task.attachment)} target="_blank" rel="noreferrer"
                            className="inline-flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-bold text-indigo-600 hover:bg-indigo-50 hover:border-indigo-300 transition shadow-sm">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
              <h2 className="text-lg font-black text-slate-800 mb-6 flex items-center gap-2">
                ...
                Discussion et Commentaires ({(task.comments || []).length})
              </h2>
              ...
              <div className="space-y-4 mb-8">
                {(task.comments || []).length === 0 ? (
                  <div className="bg-slate-50 rounded-xl border border-dashed border-slate-200 px-6 py-10 flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-xs text-slate-400">Commencez la discussion avec l'étudiant.</p>
                  </div>
                ) : (
                  (task.comments || []).map((c, idx) => (
                    <CommentBubble
                      key={c.id || idx}
                      comment={c}
                      isCurrentUser={c.encadrant_id === userData?.id || String(c.id).startsWith("temp-")}
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-6 bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
                ...
                {TASK_COLUMNS.map(c => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center gap-4 p-3 rounded-xl transition-all ${isActive ? 'bg-indigo-50 ring-1 ring-indigo-200 shadow-sm' : 'hover:bg-slate-50'}`}
                    >
              ...
              <div className="mt-8 pt-6 border-t border-slate-100">
                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">Assigné à l'étudiant</h4>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-bold text-sm text-slate-800">{student?.name || "Non assigné"}</p>
                    {student?.email && (
                      <a href={`mailto:${student.email}`} className="text-xs text-indigo-600 hover:underline">
                        {student.email}
                      </a>
                    )}
                  </div>
