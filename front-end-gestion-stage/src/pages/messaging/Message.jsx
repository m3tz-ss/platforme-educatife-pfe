...
export default function Message() {
  ...
  const handleDeleteConversation = async (convId, e) => {
    ...
    try {
      ...
      if (activeConvId === convId) {
        ...
        setReceiverId(null); receiverIdRef.current = null;
        setReceiverName(""); receiverNameRef.current = "";
        setMessages([]);
      }
  ...
  const s = {
    ...
    addContactBtn: (saved) => ({
      ...
      padding: "7px 14px", borderRadius: 10, border: "none", cursor: saved ? "default" : "pointer",
      fontSize: 12, fontWeight: 600,
      background: saved ? "#ecfdf5" : "#eef2ff",
          ...
          <div style={s.listScroll}>
            {tab === "convs" && (
              conversations.length === 0 ? (
                ...
                const deleting = deletingConvId === conv.id;
                return (
                  <div
                    key={`conv-${conv.id ?? idx}-${idx}`}
                    style={{ ...s.convItem(active), position: "relative" }}
                    onClick={() => fetchMessages(conv.id, other.id, other.name)}
                    onMouseEnter={e => {
                      if (!active) e.currentTarget.style.background = "#f8faff";
                      const btn = e.currentTarget.querySelector(".del-conv-btn");
                  ...
                  >
                    ...
                    <button
                      className="del-conv-btn"
                      title="Supprimer la conversation"
                      style={{ ...s.deleteConvBtn, opacity: 0, transition: "opacity .15s, background .12s" }}
                  ...
                  {contacts.map((c, idx) => (
                    <div
                      ...
                      onMouseEnter={e => {
                        ...
                        const btn = e.currentTarget.querySelector(".rm-contact-btn");
                        if (btn) btn.style.opacity = "1";
                      }}
                      onMouseLeave={e => {
                    ...
                    >
                      ...
                      {c.is_saved && (
                        <button
                          className="rm-contact-btn"
                          title="Retirer des contacts"
                          style={{ ...s.removeContactBtn, opacity: 0 }}
              ...
              <div style={s.chatHeader}>
                ...
                {receiverId && (
                  isContactSaved(receiverId) ? (
                    <button style={s.addContactBtn(true)} disabled>
