...
export function StudentDashboard() {
  ...
  const handleCloseModal = useCallback(() => {
    setOpenModal(false);
    // On ne reset pas selectedOffer immédiatement pour éviter un flash visuel durant l'animation de fermeture
  }, []);
        ...
        <Card className="border border-blue-gray-100 shadow-sm">
          ...
          <CardBody className="p-6 space-y-4">
            {topApplications.length === 0 ? (
              ...
              topApplications.map((app) => <ApplicationCard key={app.id} app={app} />)
            )}
          </CardBody>
        </Card>
      </div>
      ...
      <Dialog open={openModal} handler={handleCloseModal} size="lg">
        <DialogHeader className="flex justify-between items-center">
          <Typography variant="h5" className="font-bold">
            {selectedOffer?.title || "Détails de l'offre"}
          </Typography>
        ...
        <DialogFooter className="space-x-3">
          ...
          <Button
            ...
            disabled={loading || !selectedOffer || hasApplied(selectedOffer?.id)}
          >
